...
// ===== Nuova funzione setJntState aggiunta =====
void KDLRobot::setJntState(const KDL::JntArray &q)
{
    jntArray_ = q;  // Aggiorna lo stato dei giunti interni
}
...
void KDLRobot::setJntLimits(KDL::JntArray &q_low, KDL::JntArray &q_high)
{
    ...
    ikSol_ = new KDL::ChainIkSolverPos_NR_JL(chain_,
                                            *fkSol_,
                                            *ikVelSol_,
                                            100,1e-6); // Max 100 iterazioni, precisione 1e-6
